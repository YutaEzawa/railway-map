...
| 都道府県境界 GeoJSON | dataofjapan/land | `.data/japan.geojson`（gitignore） | 対象都県（埼玉・千葉・東京・神奈川）の切り出し |

入力パスは環境変数 `N02_DIR` / `JAPAN_GEOJSON` で上書き可（既定は `.data/` 配下）。

## 生成スクリプト `scripts/build-railways.cjs`

処理の流れ:

1. **対象都県ポリゴン（埼玉・千葉・東京・神奈川の和集合）の内外判定**を県境 GeoJSON から構築（ray-casting、穴対応）。
2. N02 の全フィーチャから、セグメント中点／駅重心が対象都県内のものを抽出。
3. `data/lines.csv`（`scripts/lines-csv.cjs` 経由）から **区間定義・本数**を読み込む。
...
- 定義: `build-railways.cjs` の `SERVICE_LINES`（baseN02 路線名 → サービス名・色・offset(±1)）。
...
- 単一ソースは `data/lines.csv` の `本数` 列（平日・片方向）。
...
### `public/data/railways.geojson`（路線）
...
### `public/data/stations.geojson`（駅）
